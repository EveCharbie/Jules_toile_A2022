...
def Param_variable_masse(ind_masse, Ma):
    ...
    Mressort = 0.324
    mcoin = (
        Mtoile / (Nb_ressorts_vert + Nb_ressorts_horz) + (37 / (n - 1) + 18 / (m - 1)) * Mressort / 4
    )  # masse d'un point se trouvant sur un coin de la toile
    mgrand = 1.5 * Mtoile / (Nb_ressorts_vert + Nb_ressorts_horz) + 37 * Mressort / (
        2 * (n - 1)
    )  # masse d'un point se trouvant sur le grand cote de la toile
    mpetit = 1.5 * Mtoile / (Nb_ressorts_vert + Nb_ressorts_horz) + 18 * Mressort / (
        2 * (m - 1)
    )  # masse d'un point se trouvant sur le petit cote de la toile
...
for i in range(0, 9):
    ...
    ax.set_box_aspect([1.1, 1.8, 1])
    ax.plot(
        Pt_leftfront[i][:, 0], Pt_leftfront[i][:, 1], Pt_leftfront[i][:, 2], "+r", label="Points de la toile optimisés"
    )
    ax.plot(
...
for i in range(0, 9):
    ...
    ax.plot(
        Pt_collecte_left[i][0, :], Pt_collecte_left[i][1, :], Pt_collecte_left[i][2, :], ".b", label="Points collecte"
    )
        ...
        if "t" + str(ind) in labels[p]:
            ...
            for i in range(3):
                if np.isnan(Pt_collecte_centrefront[p][i, ind_collecte1]) == False:  # gérer les nans
                    err_pos_centrefront[p] += (
                        Pt_centrefront[p][ind, i] - Pt_collecte_centrefront[p][i, ind_collecte1]
                    ) ** 2

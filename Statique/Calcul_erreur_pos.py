    ...
    for ligne in range(0, 135):
        if "t" + str(ligne) in labels[essai]:
            ...
            if (np.isnan(np.max(Pt_collecte[essai][:, ind_collecte]))) == False:
                norme.append(
                    np.sqrt(
                        (Pt[essai][ligne, 0] - Pt_collecte[essai][0, ind_collecte]) ** 2
                        + (Pt[essai][ligne, 1] - Pt_collecte[essai][1, ind_collecte]) ** 2
                        + (Pt[essai][ligne, 2] - Pt_collecte[essai][2, ind_collecte]) ** 2
                    )
                )
    normes.append(norme)
...
for i in range(0, 11):
    ...
    print(
        "erreur de position moyenne pour l essai D"
        + str(i)
        + " : "
        + str(erreur_position_moyenne)
        + " m, calculée sur "
        + str(len(normes[i]))
        + " points"
    )

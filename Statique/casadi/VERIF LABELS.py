...
def Param_variable(k_type, ind_masse):
    ...
    for j in range(1, 7):
        ...
        mmilieu = (
            2 * Mtoile / (Nb_ressorts_vert + Nb_ressorts_horz)
        )  # masse d'un point se trouvant au milieu de la toile
    ...
    if min_energie == 1:
        ...
        for i_croix in range(Nb_ressorts_croix):
            Energie += (
                ...
                * k_croix[i_croix]
                * (
                    cas.norm_fro(Spring_bout_croix_2[i_croix, :] - Spring_bout_croix_1[i_croix, :])
                    - l_repos_croix[i_croix]
                )
                ** 2
            )
...
Pt = np.reshape(Solution[17:], (n * m, 3))
print("k = " + str(k))
print("M = " + str(M))
end_main = time.time()
